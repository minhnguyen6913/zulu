...
class NewsModel extends BaseModel
{
    ...
    function _getTagNews($news_id) {
        $this->builder('tag');
        $this->builder->select('*');
        $this->builder->join('tag_news', 'tag.id = tag_news.tag_id');
        $this->builder->where('tag_news.news_id', $news_id);
    ...
    function _insertTagNews ($data)
    {
        $db      = \Config\Database::connect();
        $builder = $db->table('tag_news');
        $query   = $builder->insert($data);

        return $query->connID->insert_id;
    }
    function _deleteTagNews ($id)
    {
        $db      = \Config\Database::connect();
        $builder = $db->table('tag_news');
